...
@needs_node
def test_neighbours_rank_the_way_numpy_would(page_bundle):
    """The other reading of "more like this": nearest first, itself excluded."""
    rng = np.random.default_rng(7)
    base = rng.standard_normal(embeddings.DIMENSION)
    # Four stories at decreasing distance from the one being read, plus noise.
    pool = [
        {"story_id": i, "title": f"s{i}", "published_at": "2026-09-20T00:00:00",
         "centroid": pack(base + rng.standard_normal(embeddings.DIMENSION) * spread)}
        for i, spread in enumerate([0.0, 0.2, 0.4, 0.8, 1.6], start=1)
    ]
    got = run_in_node(
        "console.log(JSON.stringify({"
        " floor: NEIGHBOUR_FLOOR,"
        " ranked: neighbours(input.pool[0], input.pool, 4).map((n) =>"
        "   [n.story.story_id, n.score]),"
        " blind: neighbours({ story_id: 9, centroid: null }, input.pool, 4).length,"
        "}));",
        {"pool": pool},
    )

    ids = [i for i, _ in got["ranked"]]
    assert 1 not in ids, "a story is its own nearest neighbour and must be dropped"
    assert ids == sorted(ids), "the pool was built nearest-first; the order should hold"
    assert got["blind"] == 0, "a story with no vector must have no neighbours"

    want = [cosine(unpack(pool[0]["centroid"]), unpack(pool[i - 1]["centroid"])) for i in ids]
    assert np.allclose([s for _, s in got["ranked"]], want, atol=1e-12)
    assert all(s >= got["floor"] for _, s in got["ranked"])


@needs_node
def test_a_far_enough_story_is_not_offered_at_all():
    """The floor decides whether the row exists, not what order it is in."""
    rng = np.random.default_rng(8)
    story = {"story_id": 1, "centroid": pack(rng.standard_normal(embeddings.DIMENSION))}
    far = {"story_id": 2, "title": "unrelated", "published_at": "",
           "centroid": pack(rng.standard_normal(embeddings.DIMENSION))}
    got = run_in_node(
        "console.log(JSON.stringify({"
        " n: neighbours(input.story, [input.far]).length,"
        " cos: cosine(decodeVector(input.story.centroid), decodeVector(input.far.centroid)),"
        " floor: NEIGHBOUR_FLOOR }));",
        {"story": story, "far": far},
    )
    assert got["cos"] < got["floor"], "the fixture is not actually far apart"
    assert got["n"] == 0


@needs_node
def test_the_story_panel_offers_somewhere_to_go_next(page_bundle):
    """Through the real page. The fixture's two stories are deliberately about
    different things, so one is lent the other's vector: what is under test is
    that `fillStory` asks, prints and makes the answer openable."""
    out = boot(page_bundle, """
      const [a, b] = bundle.stories.filter((s) => s.centroid);
      b.centroid = a.centroid;
      fillStory(a.story_id);
      const panel = document.getElementById('sheet-body').innerHTML;
      console.log(JSON.stringify({
        pair: Boolean(a && b),
        found: neighbours(a, bundle.stories).map((n) => n.story.story_id),
        headed: panel.includes('Read next'),
        opens: panel.includes('data-open="' + b.story_id + '"'),
        notItself: !panel.includes('data-open="' + a.story_id + '"'),
        quiet: (() => {
          // A story whose neighbours are all far away prints no row at all.
          b.centroid = null;
          fillStory(a.story_id);
          return !document.getElementById('sheet-body').innerHTML.includes('Read next');
        })(),
      }));
    """)
    assert out["pair"], "the fixture bundle has fewer than two embedded stories"
    assert out["found"], "a story lent an identical vector found no neighbour"
    assert out["headed"] and out["opens"], out
    assert out["notItself"], "the story offered itself as its own next read"
    assert out["quiet"], "a story with no neighbours still printed the heading"
